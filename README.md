...
At my current job, I am in charge of our client's credit card analytics, but without any demographic
...
This project uses the UCI Default of Credit Card Clients dataset (30,000 Taiwanese credit card accounts from 2005)
to test that question directly, since it includes both transactional/behavioral data (payment history, bill amounts)
...
The dataset's documentation doesn't fully match the data itself. Rather than dropping or guessing these values,
I made the following decisions for each variable that experienced this:
          ...
          --PAY_0 (and PAY_2–PAY_6 columns): documented scale starts at -1 ("pay
          duly"), but the data contains a -2 value (2,759 records in PAY_0 alone) with no
          official definition. Instead of merging -2 into -1 (which would assume it means the
          same thing as "paid on time"), I kept it as its own category, since it's
          a different status entirely and conflating the two could obscure a real difference in behavior.
...
1–3 accounts). Those produce extreme-looking rates (100% default on a
single account) that aren't reliable patterns, basically just small-sample noise.
...
│   ├── clean_taiwan_cc_data.py   # Data cleaning, documented above
...
Python, pandas. Tableau Public dashboard:
